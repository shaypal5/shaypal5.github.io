...
`courses check-links` scans rendered `_site/**/*.html` for external `href`, `src`, selected SEO metadata image/canonical URL fields, and HTTP(S) URL strings in JSON-LD by default. Malformed rendered JSON-LD fails the check because URLs inside it cannot be reliably discovered. Build the site first, then run the checker:
